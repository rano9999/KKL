...
if($_GET['action'] == "table_data"){
   $query = mysqli_query($mysqli, "SELECT * FROM siswa ORDER BY nim");
   $data = array();
   $no = 1;
   while($r = mysqli_fetch_array($query)){
      ...
      $row[] = $kelas['kelas'];
      $row[] = $status;
      $row[] = '<a class="btn btn-danger" onclick="reset_login('.$r['nim'].')"><i class="glyphicon glyphicon-off"></i> Reset Login</a>';
      $data[] = $row;
      $no++;
   }
...
elseif($_GET['action'] == "reset_login"){
   mysqli_query($mysqli, "UPDATE siswa set status='off' WHERE nim='$_GET[nim]'");
}
?>