...
$ulangD = mysqli_query($mysqli, "SELECT * FROM nilai, siswa WHERE nilai.nim = siswa.nim AND siswa.validasi = 'Valid' AND nilai.kategori = '$kat' AND status = 'belum' ORDER BY nilaiD DESC LIMIT $kurang");
while ($D = mysqli_fetch_array($ulangD)) {
    // echo $D['nim'] . " = " . $D['kategori'] . $D['nilaiD'] . "<br>";
    mysqli_query($mysqli, "UPDATE nilai SET kategori = 'Dominant', status = 'ya' WHERE nim = '$D[nim]'");
}
